/* eslint-env jest */
const github = require('../github')
const tmp = require('tmp')

describe('github', () => {
  test('Validate username', async () => {
    expect(github.validUser('hello world')).toBe(false)
    expect(github.validUser('helloworld')).toBe(true)
    expect(github.validUser('-helloworld')).toBe(false)
    expect(github.validUser('hugo world')).toBe(false)
  })

  test('Fetch key', async () => {
    expect(github.getUserKey('hello world')).rejects.toThrow('Invalid username format')
    expect(github.getUserKey('hugorodrigues')).resolves.toEqual(['ssh-rsa '])
  })

  test('Save keys', async () => {
    const tempFolder = tmp.dirSync({ unsafeCleanup: true })

    // Sample data
    const keys = ['hello', 'world']

    // Save keys to disk
    const paths = await github.saveUserKeys(keys, tempFolder.name)

    // Have same lenght?
    expect(paths.lenght).toBe(keys.lenght)
  })
})
